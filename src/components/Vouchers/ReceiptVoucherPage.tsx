...
const ReceiptVoucherPage: React.FC = () => {
  ...
  const [formData, setFormData] = useState({
    ...
    description: invoiceNumber ? `استلام دفعة مقابل الفاتورة ${invoiceNumber}` : '',
  ...
  const loadVoucher = async (voucherId: string) => {
    try {
      ...
      console.log('🔍 جلب سند القبض:', voucherId);
      ...
      if (voucher.voucher_type !== 'receipt') {
        throw new Error('هذا ليس سند قبض');
      }
      ...
      console.log('✅ تم جلب سند القبض بنجاح');
    } catch (error: any) {
      console.error('❌ خطأ في جلب سند القبض:', error);
      setConnectionError(error.message || 'حدث خطأ في جلب سند القبض');
      toast.error('حدث خطأ في جلب سند القبض');
    } finally {
      setLoading(false);
    }
  };
  ...
  const handleSave = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (id) {
        ...
        toast.success('تم إضافة سند القبض بنجاح');
      }
      ...
      toast.error(`حدث خطأ في حفظ سند القبض: ${error.message}`);
    } finally {
      setSaving(false);
    }
  };
  ...
  const handleExportPDF = async () => {
    ...
    try {
      ...
      pdf.save(`سند-قبض-${formData.voucher_number}.pdf`);
    ...
    return (
      <div className="p-6 flex items-center justify-center min-h-screen">
        <div className="flex items-center space-x-2 space-x-reverse">
          ...
          <span className="text-gray-600">جاري تحميل سند القبض...</span>
        </div>
      </div>
    );
  }
      ...
      <div className="flex items-center justify-between mb-8">
        <div>
          <h1 className="text-2xl font-bold text-gray-900 mb-2">
            {id ? 'تعديل سند قبض' : 'سند قبض جديد'}
          </h1>
          <p className="text-gray-600">إنشاء وتعديل سندات القبض مع إمكانية الطباعة والتصدير</p>
        </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
        ...
        <div className="lg:col-span-1">
          <div className="bg-white rounded-xl shadow-lg border border-gray-100 p-6 sticky top-6">
            <h3 className="text-lg font-semibold text-gray-900 mb-4">معاينة سند القبض</h3>
            ...
            <div ref={printRef} className="p-4 border border-gray-200 rounded-lg">
              ...
              {/* Voucher Title */}
              <div className="text-center mb-6">
                <h1 className="text-2xl font-bold text-green-600 border-2 border-green-200 inline-block px-6 py-2 rounded">
                  سند قبض
                </h1>
